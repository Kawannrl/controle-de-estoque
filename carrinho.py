...
class CarrinhoApp(ctk.CTk):
    ...
    def atualizar_lista(self):
        ...
        for codigo, dados in self.carrinho.items():
            ...
            botao_mais = ctk.CTkButton(item_frame, text="+", width=30,
                                       command=lambda c=codigo: self.adicionar_quantidade(c))
            botao_mais.pack(side="right", padx=2)

            botao_menos = ctk.CTkButton(item_frame, text="-", width=30,
                                        command=lambda c=codigo: self.remover_quantidade(c))
